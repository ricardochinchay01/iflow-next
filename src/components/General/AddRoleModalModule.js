    ...
    const handleDeletePermission = async permissionId => {
        try {
            ...
            if (isAdminUserAndPermissions) {
                toast.error(
                    'No se puede eliminar el permiso para el usuario administrador.',
                    {
    ...
    return (
        <div>
            ...
            <div onClick={onToggle} className="cursor-pointer">
                <span className="ml-0 inline-block bg-blue-600 text-white px-2 py-1 rounded-full text-md">
                    {name}
                </span>
            </div>
            {isOpen &&
                permissions.map(permission => (
                    <label
                        key={permission.id}
                        className="block mb-2 items-center space-x-2 cursor-pointer">
                        <input
                            type="checkbox"
                        ...
                        {permission.isNew && (
                            <span className="ml-2 inline-block bg-green-200 text-green-800 px-2 py-1 rounded-full text-xs">
                                Nuevo
                            </span>
                        )}
                        <button
