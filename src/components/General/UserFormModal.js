...
const UserFormModal = ({
    ...
    const validateForm = () => {
        ...
        } else if (Object.keys(formData).length !== 0 && formData.email !== formData.email) {
            // Aquí podrías agregar otras validaciones específicas para el email cuando se cambia.
        }
        if (!formData.role) {
            errors.role = "El rol es obligatorio.";
        } else if (Object.keys(formData).length !== 0 && formData.role !== formData.role) {
    ...
    const userRole = initialData.roles && initialData.roles[0] ? initialData.roles[0].id : '';
    ...
    return (
        <div className="fixed top-0 left-0 w-full h-full flex items-center justify-center z-50">
            ...
            <div className="bg-white w-3/4 max-w-xl rounded p-4 z-10">
                ...
                <h2 className="text-xl mb-4">{Object.keys(initialData).length === 0 ? "Nuevo Usuario" : "Editar Usuario"}</h2>
                <form onSubmit={handleSubmit}>
                    ...
                    <ValidatedSelect
                        ...
                        placeholder="Rol"
                        value={formData.role}
                        onChange={(e) => setFormData((prevState) => ({ ...prevState, role: e.target.value }))}
                    ...
                    <div className="flex justify-between items-center">
                        ...
                        <button type="submit" className="bg-blue-900 text-white px-5 py-2 rounded-lg shadow hover:bg-blue-800">
                            {Object.keys(initialData).length === 0 ? "Guardar Usuario" : "Actualizar Usuario"}
                        </button>
                    </div>
                </form>
            </div>
