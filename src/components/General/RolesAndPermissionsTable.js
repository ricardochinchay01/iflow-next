    ...
    return (
        <table className="min-w-full bg-white rounded-lg overflow-hidden shadow-md">
            <thead className="bg-blue-900 text-white">
                <tr>
                    <th
                        className="py-3 px-6 text-center w-1/24 cursor-pointer"
                        onClick={() => changeSorting('id')}>
                        <span className="mr-2">Id</span>
                        {paginationData.column === 'id' &&
                    ...
                    <th
                        className="py-3 px-6 text-center w-1/6 cursor-pointer"
                        onClick={() => changeSorting('name')}>
                        <span className="mr-2">Rol</span>
                        {paginationData.column === 'name' &&
                            (paginationData.orderBy === 'asc' ? (
                                <FontAwesomeIcon icon={faArrowUp} />
                            ) : (
                                <FontAwesomeIcon icon={faArrowDown} />
            ...
            <tbody>
                {roles.map(role => {
                    const modules = role.permissions.reduce(
                        (acc, permission) => {
                            const [
                                moduleName,
                                permissionName,
                    ...
                    return (
                        <tr key={role.id} className="border-b text-gray-700">
                                        ...
                                        <div
                                            ...
                                            {expandedModule === moduleName &&
                                                permissions.map(permission => (
                                                    <span
                                                        key={permission}
                                                        className="mr-2 mb-2 bg-red-600 text-white px-4 py-1 rounded-full text-xs">
                                                        {permission}
                                                    </span>
                                                ))}
                                        </div>
                            ...
                            <td className="py-3 px-6 text-center">
                                <strong>{formatDate(role.updated_at)}</strong>
                            </td>
                            <td className="py-3 px-6 text-center">
                                <Link
                                    href={`/roles/${role.id}`}
