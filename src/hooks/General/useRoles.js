...
const useRoles = () => {
    ...
    const fetchRolesWithPermissions = async (currentPage, perPage = 10, column = 'id', orderBy = 'desc') => {
        try {
            setLoading(true);
            const fetchedRoles = await axios.get(`/api/roles?current_page=${currentPage}&per_page=${perPage}&order_by=${orderBy}&column=${column}&moduleId=${filterData.moduleId}`);
            const { data } = fetchedRoles;
    ...
    const handleFilter = (e) => {
        e.preventDefault();
        fetchRolesWithPermissions(1, paginationData.perPage, paginationData.column, paginationData.orderBy);
    };
    ...
    const deleteRole = async (roleId) => {
        const isConfirmed = window.confirm('¿Estás seguro de que deseas eliminar este rol? Se eliminará el rol y se revocarán los permisos asociados al mismo.');
        ...
        try {
            ...
            setRolesWithPermissions(prevRoles => prevRoles.filter(role => role.id !== roleId));
        } catch (error) {
    ...
    const updateRolePermissions = async (roleId, updatedRole) => {
        try {
            // await csrf();  // Obtener el token CSRF antes de hacer la petición
            const response = await axios.put(`/api/roles/${roleId}/permissions`, updatedRole);
            return response.data; // Puedes retornar los datos de respuesta si lo necesitas
        } catch (error) {
    ...
    useEffect(() => {
        fetchRolesWithPermissions(paginationData.currentPage, paginationData.perPage, paginationData.column, paginationData.orderBy);
